...
export interface CheckoutReplayClient {
  checkoutReplay: {
    ...
    createMany(args: { data: { customerId: string; key: string; requestHash: string; status: 'IN_PROGRESS' }[]; skipDuplicates: true }): Promise<{ count: number }>;
    update(args: { where: Key; data: { status: 'COMPLETED'; response: unknown } }): Promise<unknown>;
  };
}

export class PrismaCheckoutReplay implements Pick<IdempotencyStore, 'find' | 'reserve' | 'complete'> {
  ...
  find(customerId: string, key: string): Promise<IdempotencyRecord | null> {
    return this.clients.current().checkoutReplay.findUnique({ where: { customerId_key: { customerId, key } } });
  }
  ...
  async reserve(customerId: string, key: string, requestHash: string): Promise<boolean> {
    const { count } = await this.clients.current().checkoutReplay.createMany({
      data: [{ customerId, key, requestHash, status: 'IN_PROGRESS' }], skipDuplicates: true,
    });
    return count === 1;
  }

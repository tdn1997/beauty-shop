...
export function validateCheckoutRequest(body: unknown): CheckoutRequest {
  const invalid = () => {
    throw new DomainError(
      ...
      'Expected addressId, VND currency and unique variant quantities',
    );
  };
  if (!body || typeof body !== 'object' || Array.isArray(body)) return invalid();
  const value = body as Record<string, unknown>;
  if (Object.keys(value).some((key) => !['addressId', 'currency', 'lines'].includes(key)))
    return invalid();
  if (
  ...
  const lines = value.lines.map((line: unknown) => {
    ...
    if (
      Object.keys(entry).some((key) => !['variantId', 'quantity'].includes(key)) ||

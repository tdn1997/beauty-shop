...
@Module({
  ...
  providers: [
    ...
    {
      provide: AuthService,
      inject: [PrismaService, ScryptPasswordHasher],
      useFactory: async (p: PrismaService, h: ScryptPasswordHasher) =>
        new AuthService(
          new PrismaAuthStore(p),
          h,
          () => new Date(),
          await h.hash('dummy-password-never-login'),
        ),
    },
    AuthGuard,
    RolesGuard,

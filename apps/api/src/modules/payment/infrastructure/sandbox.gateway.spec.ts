...
function clientReturning(status: number, body: unknown): SandboxHttpClient {
  return { post: vi.fn(async () => ({ status, body })), get: vi.fn(async () => ({ status, body })) };
}

describePaymentGatewayContract('SandboxGateway', (status) => {
  const body = { status: status === PaymentStatus.Failed ? 'DECLINED' : status, reference: 'sandbox_1', redirectUrl: null, reason: status === PaymentStatus.Failed ? 'Thẻ bị từ chối' : null };
  return { gateway: new SandboxGateway({
    post: async () => ({ status: 200, body }),
    get: async (url) => url.endsWith('/missing') ? { status: 404, body: null } : { status: 200, body },
  }, 'https://sandbox.test') };
});
...
describe('SandboxGateway - wire mapping', () => {
  ...
  it('should transmit exact decimal strings and encode reference paths', async () => {
    // arrange
    const client = clientReturning(200, { status: 'PENDING', reference: 'ref/1', redirectUrl: 'https://sandbox.test/pay', reason: null });
    const gateway = new SandboxGateway(client, 'https://sandbox.test/');
    // confirm
    ...
    expect(client.post).toHaveBeenCalledWith('https://sandbox.test/payments', { orderId: 'order_1', amount: '9007199254740993', currency: 'VND', returnUrl: initiation.returnUrl });
  ...
  it('should map an initiation timeout to UNKNOWN because money may already have moved', async () => {
    ...
    client.post = vi.fn(async () => { throw timeout; });
    const gateway = new SandboxGateway(client, 'https://sandbox.test');
    // confirm
    expect(client.post).not.toHaveBeenCalled();
    // act
  ...
  it('should propagate connection failures and query timeouts without changing the request', async () => {
    ...
    const timeout = Object.assign(new Error('timeout'), { code: 'ETIMEDOUT' });
    const client: SandboxHttpClient = { post: async () => { throw failure; }, get: async () => { throw timeout; } };
    const gateway = new SandboxGateway(client, 'https://sandbox.test');
